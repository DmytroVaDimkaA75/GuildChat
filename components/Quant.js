...
const tap=(n,g)=>{if(multiIds.size||scheduledReview){if(multiIds.has(String(n.id))){removeMultiple(n.id);return}if(!blocked(n)){clearSelection();return}if(avoid(n)||(!scheduledReview&&scheduled(n)))return;if(gradientType(n)==='standard')clearSelection();else addMultiple(n,g);return}clearNormal();g.classList.add('selected');send('nodePress',String(n.id))};
