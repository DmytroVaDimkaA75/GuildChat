...
const MAP_DIMENSIONS = { [DEFAULT_MAP_KEY]: { width: VOLCANIC_SVG_WIDTH, height: VOLCANIC_SVG_HEIGHT }, waterfall_archipelago: { width: WATERFALL_SVG_WIDTH, height: WATERFALL_SVG_HEIGHT } };
...
const GVG = () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            } catch (error) { if (isActive) setShortGuildId(null); }
        })();
        ...
        return Object.entries(data).map(([sectorId, config]) => {
            ...
            return (
                <G key={sectorId} onPress={() => handleShapePress(sectorId)}>
                    {fill && <AnimatedPath {...fill.props} d={fill.d} onPressIn={(e) => handleShapePress(sectorId, e)} style={fillStyle} fillOpacity={animatedFillOpacity} />}
                ...
                <View style={styles.infoOverlay}>
                    ...
                    <Animated.View style={styles.infoModal}>
                        ...
                        <ScrollView style={styles.infoList}>
                            {opponentList.length === 0 ? (<Text style={styles.infoEmpty}>Інформація відсутня</Text>) : (
                                opponentList.map(op => (
...
const styles = StyleSheet.create({
    ...
    sectorRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 12, paddingHorizontal: 15, backgroundColor: '#282828', borderRadius: 12, marginBottom: 10, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.2, shadowRadius: 1.41 },
