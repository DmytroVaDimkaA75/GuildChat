...
const AppContent = () => {
  ...
  // ✅ Пуші: permissions + канал + збереження токена + foreground показ (як у тебе було)
  useEffect(() => {
    const setupPushNotifications = async () => {
      try {
        if (Platform.OS === 'ios') {
          const authStatus = await messaging().requestPermission();
          // нічого не ламаємо навіть якщо відмовив
    ...
    const unsubscribeForeground = messaging().onMessage(async remoteMessage => {
      try {
        ...
        // Якщо це data-only без body/title — не показуємо
